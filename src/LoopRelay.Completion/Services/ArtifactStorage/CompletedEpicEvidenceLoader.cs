...
public sealed class CompletedEpicEvidenceLoader(CompletionArtifacts artifacts)
{
    ...
    public static string Render(IReadOnlyList<CompletedEpicEvidence> completedEpics)
    {
        if (completedEpics.Count == 0)
        {
            ...
            No completed epic markdown files were found under `{CompletionArtifactPaths.CompletedEpicsPattern}`.
            """;
        }
        ...
        builder.AppendLine();
        builder.AppendLine($"Completed epic source glob: `{CompletionArtifactPaths.CompletedEpicsPattern}`");
    ...
    private static string ExtractRenderedContent(string content)
    {
        ...
        return Limit(
            extracted.Trim(),
            MaxRenderedContentPerEpic,
            "_Per-epic evidence truncated because this archived epic exceeded the per-file evidence budget._");
    }
    ...
    private static string ExtractKnownSections(string content)
    {
        ...
        {
            if (!TryParseHeading(lines[index], out int level, out string? heading) ||
                !IsEvidenceSection(heading))
            {
                continue;
            }
    ...
    {
        foreach (IReadOnlyDictionary<string, string> row in CompletionMarkdownTableParser.ParseTables(content))
        {
