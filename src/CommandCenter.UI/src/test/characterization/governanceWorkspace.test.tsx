...
describe('governance workspace characterization', () => {
  it('renders lifecycle explanation, transfer readiness, recovery, health, and certification facts', () => {
    ...
    expect(within(recovery).getByLabelText('Recovery Interaction Summary')).toBeInTheDocument()
    expect(within(recovery).getByText('Decision-session recovery requiring review')).toBeInTheDocument()
    expect(within(recovery).getByText('Recovery recovery-1 requires review.')).toBeInTheDocument()
    expect(within(recovery).getByText('Recover governance')).toBeInTheDocument()
    expect(within(recovery).getByText('Recovery run')).toBeInTheDocument()
    expect(within(recovery).getByText('Transfer assessment: Interrupted')).toBeInTheDocument()
    expect(within(recovery).getAllByText('Duplicate active sessions require intervention.').length).toBeGreaterThan(0)
    expect(within(recovery).getAllByText('Transfer did not complete.').length).toBeGreaterThan(0)
    expect(within(recovery).queryByText('Recovered: No')).not.toBeInTheDocument()
