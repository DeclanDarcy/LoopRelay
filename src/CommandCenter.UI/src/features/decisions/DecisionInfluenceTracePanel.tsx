...
import { DecisionInfluenceExplorer } from './DecisionInfluenceExplorer'

type DecisionInfluenceTracePanelProps = {
...
export function DecisionInfluenceTracePanel({
  trace,
  isLoading = false,
  error = null,
}: DecisionInfluenceTracePanelProps) {
  ...
  return (
    <section className="decision-lifecycle-panel decision-influence-trace-panel" aria-label="Decision influence trace">
      <div className="decision-panel-heading">
        <h5>Execution Influence Trace</h5>
        <span>{trace ? `${decisionIds.length} influencing decisions` : 'No trace loaded'}</span>
      </div>
