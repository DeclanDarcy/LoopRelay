...
function createReasoningMaterializationReview(
  ...
  return {
    ...
    taxonomyFindings: Object.entries(familyCounts).map(([family, count]) => ({
      ...
      terminalEventTypes: [],
      riskReason: `Lifecycle risk is not flagged because ${family} does not meet the mock threshold basis.`,
      ...
      decisions: [
        {
          ...
          taxonomyBasis: {
            ...
            matchedEvidence: ['Matched architectural keyword: Backend services own'],
          ...
          omissionReason: null,
          operationalStatement: 'Decision: Backend services own proposal generation and persistence because review state must be authoritative.',
          rationale: 'review state must be authoritative',
          constraintsIntroduced: ['Generated proposals do not mutate current operational context.'],
          consequencesIntroduced: ['Review UX must show proposal persistence as backend-owned state.'],
          openQuestions: [],
          sourceEvidence: [
            'Source artifact: .agents/decisions/decisions.md',
            'Decision statement: Backend services own proposal generation and persistence because review state must be authoritative.',
          ],
        },
      ],
      consequences: [
        {
          consequenceId: 'mock-consequence-1',
          originatingDecision: {
            decisionId: 'DEC-MOCK-0001',
            sourceRelativePath: '.agents/decisions/decisions.md',
            statement: 'Backend services own proposal generation and persistence because review state must be authoritative.',
          ...
          operationalStatement: 'Review UX must show proposal persistence as backend-owned state.',
          affectedArea: 'Operational context review',
          supportingEvidence: ['Consequence statement: Review UX must show proposal persistence as backend-owned state.'],
