...
internal sealed class PlanWarmSessionContinuityStore(Repository _repository)
{
    private const string MetadataKey = "plan_warm_session.v1";
    private static readonly JsonSerializerOptions Options = new(JsonSerializerDefaults.Web);

    public async Task WriteAsync(PlanWarmSessionContinuity continuity, CancellationToken cancellationToken)
    {
    ...
    public async Task<PlanWarmSessionContinuity?> ReadAsync(CancellationToken cancellationToken)
    {
        string database = LoopRelayWorkspaceDatabase.Resolve(_repository);
        ...
        try
        {
            return JsonSerializer.Deserialize<PlanWarmSessionContinuity>(json, Options);
        }
        catch (JsonException)
        {
            return null;
