...
/// transfer). Uses the same <c>Path.GetTempPath()/CommandCenter.*/{guid}</c> convention as the rest of the code.
/// </summary>
public sealed class TempSandboxWorkspaceFactory : ISandboxWorkspaceFactory
{
    public Task<ISandboxWorkspace> CreateAsync(string label, CancellationToken cancellationToken = default)
    {
        ...
        string root = Path.Combine(Path.GetTempPath(), "CommandCenter.Sandbox", Guid.NewGuid().ToString("N"));
