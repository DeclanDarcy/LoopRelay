...
Status: defined and inventoried at the contract-family level; locally certified for the repository dashboard and repository workspace pilots.
...
| Oracle change workflow | `docs/contracts.md` defines the procedural workflow for classifying fixture comparison, consumer verification, artifact freshness, and request-boundary drift before accepting a new baseline. It names the required change record, canonical sequence, acceptance rules, repository pilot commands, evidence requirements, and rollback requirement. | The workflow is procedural, not automated. It does not assign versions mechanically, generate artifacts, update freshness manifests, or certify additional contract families beyond the repository dashboard and repository workspace pilots. |
...
| Workflow projection field inventory | `.agents/milestones/m0.2-workflow-projection-field-inventory-slice-0026.md` identifies `WorkflowInstance` as the next primary Oracle contract, maps producer, request boundary, shell pass-through, TypeScript type/API consumers, UI consumers, absent dev mock handler, top-level semantic field groups, fixture candidate requirements, and sibling endpoint limits. | No workflow fixture, consumer verification, artifact freshness, request-boundary verifier, or local certification exists yet. |
| Workflow fixture field classification | `.agents/milestones/m0.2-workflow-fixture-field-classification-slice-0027.md` classifies each top-level `WorkflowInstance` field as semantic authority, structural metadata, compatibility field, diagnostic field, or derived presentation helper; defines nested path classification rules; and blocks fixture capture until explicit nulls, diagnostics arrays, ordered arrays, backend eligibility booleans, and flattened compatibility fields are represented. | Classification is a review gate only. It does not add a fixture, verifier, generated artifact, or consumer proof. |
