...
1. **Write / Revise plan** (held-open Operational planning session). `BeginWritePlanAsync` persists the Roadmap to `.agents/specs/roadmap.md` and each Spec to `.agents/specs/s{n}.md` (1-based) *before* the turn, then runs `WritePlanForNewCodebase` or `WritePlanAgainstCodebase` (selected by `newCodebase`) on the warm session, streaming to the plan stream; on completion it verifies `.agents/plan.md` exists, caches it, and emits `completed`. `BeginRevisePlanAsync` runs `RevisePlan` against the same warm session.
...
- It computes `RouterInputs` (observed decision-session tokens when `> 0`, otherwise a deterministic `(len + 3) / 4` estimate over the handoff plus decisions) and calls the **registry-free** `DecisionSessionRouter.Evaluate`, which thresholds `DecisionSessionTokens` against `DecisionSessionRouterOptions.DecisionTokenTransferThreshold` (default `200_000`). The router performs no I/O and consults no registry policy or eligibility service.
