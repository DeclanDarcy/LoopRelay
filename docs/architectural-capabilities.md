...
| Architectural decision governance | 0.4 | Decision governance reference document, architectural evidence model, decision record template, decision class catalog, mechanism lifecycle approval rules, emergency exception rules, rollback policy, baseline update policy, backend governance metadata guard, and architecture-regression bypass guard | Opening governance definition guard verified locally in Slice 0050; architecture-regression bypass guard verified locally in Slice 0051; milestone certification remains pending | `docs/architecture-decision-governance.md`, `docs/architectural-evidence.md` | Introduced as a guarded definition slice with initial regression-weakening enforcement; M0.4 still needs broader ungoverned-change detection, active decision/evidence validation, certification, and baseline acceptance |
