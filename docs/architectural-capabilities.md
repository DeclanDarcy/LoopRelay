...
| Architectural decision governance | 0.4 | Decision governance reference document, architectural evidence model, decision record template, decision class catalog, mechanism lifecycle approval rules, emergency exception rules, rollback policy, baseline update policy, backend governance metadata guard, architecture-regression bypass guard, and shell Rust struct classification guard | Opening governance definition guard verified locally in Slice 0050; architecture-regression bypass guard verified locally in Slice 0051; shell Rust struct classification guard verified locally in Slice 0052; milestone certification remains pending | `docs/architecture-decision-governance.md`, `docs/architectural-evidence.md`, `docs/shell-transport-classification.md` | Introduced as a guarded definition slice with initial regression-weakening and shell mirror governance enforcement; M0.4 still needs broader ungoverned-change detection, active decision/evidence validation, certification, and baseline acceptance |
