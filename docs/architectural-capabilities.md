...
| Architectural decision governance | 0.4 | Decision governance reference document, architectural evidence model, decision record template, decision class catalog, mechanism lifecycle approval rules, emergency exception rules, rollback policy, baseline update policy, and backend governance metadata guard | Opening governance definition guard verified locally in Slice 0050; milestone certification remains pending | `docs/architecture-decision-governance.md`, `docs/architectural-evidence.md` | Introduced as a guarded definition slice; M0.4 still needs compatibility/regression-weakening policy expansion, ungoverned-change detection, certification, and baseline acceptance |
