...
| m8-frozen orchestration-loop run-event types (`src/CommandCenter.UI/src/types/planning.ts`, `executionRun.ts`, `decisionRun.ts`) and the generated `src/CommandCenter.UI/src/contracts/generated/repository-dashboard.generated.ts` | Compatibility mirror | Orchestration contract Oracle (m8 freeze) | Orchestration-loop UI hooks/streams (`usePlanStatus`, `usePlanStream`, `useExecutionStream`, `useDecisionStream`) and the repository dashboard projection | These TypeScript shapes mirror backend-owned loop contracts and must stay byte-identical; the backend producers (`PlanStatus`/`PlanLifecycleState`, the plan/execution/decision stream events) are the authority and may only extend the shape additively. | Retire the freeze only when a generated-contract pipeline regenerates these shapes from the backend producers and the consumer/freshness suites prove no field loss; until then the byte-freeze is the compatibility guarantee. | `tests/CommandCenter.Backend.Tests/OrchestrationConsumerContractTests.cs`, `ContractGeneratedArtifactPipelineTests.cs` |
