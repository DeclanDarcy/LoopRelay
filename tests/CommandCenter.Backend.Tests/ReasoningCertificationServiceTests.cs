...
public sealed class ReasoningCertificationServiceTests
{
    ...
    [Fact]
    public async Task CertificationEndpointsReturnCurrentPersistedRunAndHistory()
    {
        Repository repository = CreateRepository();
        ...
        HttpResponseMessage currentResponse = await client.GetAsync(
            $"{root}/api/repositories/{repository.Id}/reasoning/certification");
        HttpResponseMessage runResponse = await client.PostAsync(
            $"{root}/api/repositories/{repository.Id}/reasoning/certification",
            null);
        HttpResponseMessage historyResponse = await client.GetAsync(
            $"{root}/api/repositories/{repository.Id}/reasoning/certification/reports");

        ReasoningCertificationReport current =
            (await currentResponse.Content.ReadFromJsonAsync<ReasoningCertificationReport>(JsonOptions))!;
        ReasoningCertificationReport persisted =
            (await runResponse.Content.ReadFromJsonAsync<ReasoningCertificationReport>(JsonOptions))!;
        ReasoningCertificationReport[] history =
            (await historyResponse.Content.ReadFromJsonAsync<ReasoningCertificationReport[]>(JsonOptions))!;
