...
public sealed class ProjectionCacheTests
{
    ...
    public async Task Existing_projection_does_not_call_runtime()
    {
        ...
        ProjectionCacheResult result = await cache.EnsureAsync("SelectNextEpic", northStar, new PromptContractRegistry(new ProjectionRegistry()).Get("SelectNextEpic"), CancellationToken.None);
    ...
    public async Task Failed_turn_does_not_write_projection()
    {
        ...
        await Assert.ThrowsAsync<RoadmapStepException>(() => cache.EnsureAsync("SelectNextEpic", northStar, new PromptContractRegistry(new ProjectionRegistry()).Get("SelectNextEpic"), CancellationToken.None));
    ...
    public async Task Stale_projection_blocks_when_contract_policy_is_block()
    {
        ...
        await new ProjectionManifestStore(repo.Artifacts).UpsertAsync(new ProjectionManifestEntry("SelectNextEpic", "ProjectionForSelectNextEpic", path, "p", RoadmapArtifactPaths.NorthStarSourceFiles, "old", "h", DateTimeOffset.UtcNow, ProjectionValidationStatus.Valid, ProjectionStaleStatus.Fresh, null));
        NorthStarContext northStar = await new NorthStarContextLoader(repo.Artifacts).LoadAsync();
