...
public sealed class FinalAcceptanceTests
{
    ...
    public void Orchestration_loop_does_not_absorb_the_reasoning_or_knowledge_subsystem()
    {
        ...
        // per-repository reasoning-graph subsystem (LoopRelay.Reasoning) and the per-repository decision
        // discovery/recommendation services (LoopRelay.Decisions) are pre-existing, repository-scoped, and
        ...
        Assert.DoesNotContain("LoopRelay.Reasoning", referenced);
        ...
        Assert.Contains("LoopRelay.Agents", referenced);
    }
    ...
    {
        // FA-12 / NG-5: all prompt text comes from the generated LoopRelay.Core.Prompts classes, which are
        ...
        {
            Assert.Equal("LoopRelay.Core.Prompts", promptType.Namespace);
