...
public sealed class OperationalPolicyResolverTests
{
    ...
    [Fact]
    public void Resume_policy_is_identified_and_workspace_recovery_strategy_is_governed()
    {
        ResolvedOperationalPolicy reconstructed = Resolve(
            workspace: new CliPolicyDocument(null, null, null, true, "reconstructed"));
        ResolvedOperationalPolicy certified = Resolve(
            workspace: new CliPolicyDocument(null, null, null, true, "certified"));
        ...
        Assert.Equal(PolicyLayer.Workspace, provenance.Layer);
    }
    ...
    public void Shipped_default_template_matches_the_built_in_policy_defaults()
    {
        ...
        Assert.Equal(
            ...
            template.PolicyInputs.MaxUnboundedContinuationSteps);
        Assert.Equal(
            OperationalPolicyResolver.DefaultMaxNoChangesCommits,
            template.PolicyInputs.MaxNoChangesCommits);
        Assert.Equal(
            OperationalPolicyResolver.DefaultOperationalContextGrowthWarningStreak,
            template.PolicyInputs.OperationalContextGrowthWarningStreak);
        Assert.Equal(
