...
launch goes unrecorded. The **prerequisite doctor** consumes the resolved
host-runtime profile (runtime-profile identity plus declared provider capabilities),
routes it through the selected provider's inspector, and emits an immutable typed
inspection fact; it never reads policy inputs. The Codex inspector checks
