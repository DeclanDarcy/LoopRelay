...
The roadmap also adds prompt guidance so planning and execution avoid autonomously creating non-implementation files unless the HITL specifically requested them.
...
* Non-implementation documentation is allowed only when the HITL specifically requested it.
...
* the repository policy setting that controls implementation-first guidance
...
Conditionally inject implementation-first guidance so planning and execution prompts avoid generating non-implementation artifacts unless the HITL specifically requested them.
...
When enabled, guidance should discourage:
...
The guidance remains conditional. If the HITL specifically requests non-implementation documentation, planning and execution should honor that request.
