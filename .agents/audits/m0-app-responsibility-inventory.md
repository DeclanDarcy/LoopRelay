...
| Repository list container and selected repository summary | Presentation plus navigation, registration/removal affordances, selection reconciliation, and availability/readiness display. | Retain container/selectable item ownership in `App.tsx`; selected repository summary may be audited separately only if action affordances and refresh/remove coordination stay out of the extracted component. |
