...
- Current operational-context display rendering is now extracted to `OperationalContextCurrentPanel`; proposal summary display is now extracted to `OperationalContextProposalSummaryPanel`; proposal compression summary display is now extracted to `OperationalContextCompressionSummaryPanel`; proposal review toolbar/actions, proposal draft editing, review note ownership, semantic change display, comparison rendering, and promotion/review command gating remain composed in `App.tsx`.
...
| Operational-context proposal review | Workflow coordination plus local draft/review state and backend mutation gating. | Retain review controls, draft fields, semantic-change display, decision review, and comparison rendering in `App.tsx` unless a later audit proves another presentation-only boundary. |
