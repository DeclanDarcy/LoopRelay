...
- Git workflow evidence summaries are now extracted to `CommitPreparationSummary`, `PushReviewSummary`, and `GitStatusDetails`; commit preparation loading, commit message draft ownership, selected commit path draft ownership, commit readiness, push readiness, refresh, commit, and push command ownership remain composed in `App.tsx`.
- Execution workflow rail rendering is now extracted to `ExecutionWorkflowRail`; execution workflow step derivation, selected milestone display, execution-state badge, context-build/start controls, and launch readiness remain composed in `App.tsx`.
...
| Git commit/push evidence summaries | Pure presentation of caller-owned commit preparation metadata, push review metadata, and git status details/path buckets. | Extracted to `CommitPreparationSummary`, `PushReviewSummary`, and `GitStatusDetails`; tests pin labels, fallback text, and dirty-path bucket rendering. |
| Git commit/push review controls | Workflow coordination plus preparation loading, commit draft, selected paths, preparation currency, commit readiness, push readiness, refresh, commit, and push command ownership. | Retain in `App.tsx`; do not move without preserving explicit action authority and local draft boundaries. |
| Execution workflow rail | Pure presentation of caller-computed lifecycle steps. It preserves provided ordering, labels, details, and state classes without deriving readiness or invoking workflow commands. | Extracted to `ExecutionWorkflowRail`; tests pin caller-provided order, state classes, and detail text. |
| Execution workspace controls | Selected milestone display, execution-state badge, context-build action, launch action, and launch readiness surface. | Retain in `App.tsx`; this region couples navigation selection, backend preview construction, explicit execution launch, and readiness feedback. |
...
## Final M0.5 Scan Result

The final deliberate scan found one remaining high-value neutral presentation island, the execution lifecycle rail, and extracted it. The remaining large `App.tsx` JSX regions are intentionally retained for M0 because they own one or more of navigation state, local draft state, explicit backend workflow actions, workflow readiness, post-mutation projection reconciliation, selection composition, or review guidance.

No further extraction is authorized in M0.5 solely to reduce line count. Additional decomposition should happen with later workspace-feature migrations where workflow and draft boundaries can move as coherent units.
