...
- [x] Wire settings and request capture into:
  - [x] `LoopCliComposition`
  - [x] `PlanCliComposition`
  - [x] `RoadmapCliComposition`
  - [x] completion prompt runner composition where needed
- [x] Prompt rendering changes:
  - [x] `WritePlan.prompt`: include policy guidance and require plans to avoid autonomous non-implementation deliverables; if the setting is enabled and the HITL explicitly requested a non-implementation deliverable, list it in a structured HITL-request section.
  - [x] `RevisePlan.prompt`: carry the same policy and preserve/update only explicitly grounded HITL-request entries.
  ...
  - [x] completion prompts should receive review summaries when present.
...
- [x] Add tests:
  ...
  - [x] no prompt has a separately hard-coded copy of the policy body outside the composer
...
## Acceptance
- [x] Relevant prompts receive implementation-first guidance from one composition point.
- [x] Default planning and execution steer away from autonomous non-implementation files.
- [x] Enabled mode allows non-implementation deliverables only when the HITL specifically requested them.
- [x] HITL request evidence is captured when granted, not reconstructed only at completion.
