...
- [x] Update freshness and prompt consumers to use logical resolution for any path that can become SQLite-backed.
...
- [x] Update unblock evidence hashing for execution evidence and migrated histories.
...
- [x] Hash drift in retained and migrated domains reports stale.
- [x] Missing migrated evidence reports stale, invalid, or blocked according to consumer behavior.

## Exit Criteria

- [x] File-backed freshness results match current tests.
- [x] All path references that may later point to SQLite-backed records resolve through the logical resolver.
